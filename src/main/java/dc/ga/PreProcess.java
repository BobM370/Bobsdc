...
public class PreProcess {
	...
	public void buildTest(Event[] testEvents) {
		...
		for (int testIt = lastTrainingEvent.end + 1; testIt < lengthOfDataset; testIt++)
		// we only want records in the testing data.
		{
	...
	public void extractTestDCfeatures(Event[] testEvents) {
		...
		for (int k = 0 /* testIndexStart */; k < testEvents.length  /* testIndexEnd */; k++) {
			...
			double percentagePriceDisplacement = price; // (price / endPrice) * 100.0;
	...
	void getAutoWekaClassifierDetailResult() {
		...
		return;
	} // end of getAutoWekaClassifierDetailResult
...
//			int[] stats = trainingInstances.attributeStats(trainingInstances.classIndex()).nominalCounts;
	...
	} // end of extractDCTrainingfeatures
...
//			Classifier classifier = (Classifier) pair.getKey()[0];
			...
			double fmeasureCopy = 5.0 * (((precision) * (recall)) / ((4 * precision) + (recall)));
			// ((2*((pricisionDouble* recall)/(pricisionDouble+ recall)) *0.7) + (0.3* filterAccuracyDbl));
...
//					.stringValue(trainingInstances1.attribute(trainingInstances1.numAttributes() - 1));
	...
	} // end of printPreprocessClassificationTraining
...
//					.stringValue(testInstances.attribute(testInstances.numAttributes() - 1));
	...
	} // end of printPreprocessClassification
	...
	public void cleanupAutoWekaTemp(String tempPath, int position) {
		...
		if (System.getProperty("os.name").contains("Windows")) {
			...
			// ");
			try {
				String path = tempPath.replaceAll("\\\\", "\\\\\\\\");
				// need to double up to work
	...
	} // end of deleteDirectoryRecursionJava6
	...
	String getBestAutoWekaTempFolderPrefix() {
		...
		return prefix;
	} // end of getBestAutoWekaTempFolderPrefix
	...
	} // end of deleteAllTempFilesExceptBestAutoClassifier
}