...
public class DCCurve {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < curves.length; i++) {
			...
			// here we are generating the events for the training set,
			// you have to make a separate call for the test set.
	...
	public void build(Double[] values, double delta, String GPTreeFileName, boolean isTraining) {
		...
		// getGPTreeForCurve(GPTreeFileName);
		// Adesola Get the best GP for this curve while reading GP from hard drive
		ScriptEngineManager mgr = new ScriptEngineManager();
		ScriptEngine engine = mgr.getEngineByName("JavaScript");
		...
		for (double value : values) {
			...
			/*
			 * If tree is read from file use this. if this fails remove and rollback to previous
			 */
			output[index - 1] = last;
	...
	protected Event detect(Type type, int[] indexDC, int[] indexOS) {
		// overshoot event must have a start index lower that the DC event start index
