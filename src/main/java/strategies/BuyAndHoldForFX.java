/** This is a simple Buy and Hold strategy. We buy at the beginning
* of the specified period, and sell at the very end **/
package strategies;
...
public class BuyAndHoldForFX {
	...
	Fitness fitness()
	{
		double cash = budget; // set my initial cash to always be equal to a fixed amount,
		// for both training and testing sets.
		double stock = 0;
		double shortSellingStock = 0;//when I go short, I borrow stocks from a broker,
		// which I immediately sell. So my shortSellingInventory goes negative, because I'm now 'short' of
		...
		// My fitness is equal to my wealth = cash + value of stocks I owe either due to buying
		// or short-selling activity. If, however, I end up with a negative shortSellingStock number; 
		// so in this occasion the value of my short selling stocks is subtracted from my total fitness,
		// as I need to re-buy the stocks to return them to my broker.
		fitness.value = OpeningPosition - 500000.0 ; // realisedProfit;
		fitness.realisedProfit = OpeningPosition - 500000.0 ; //realisedProfit;
		fitness.wealth = OpeningPosition; // realisedProfit + budget; 
		// my wealth, at the end of the transaction period
	...
	public static void main(String[] args) throws NumberFormatException, FileNotFoundException, IOException {
		...
		BuyAndHoldForFX bh = null;
		 ...
		 if (s.length == 5)
			bh = new BuyAndHoldForFX(s[0], s[4], Integer.parseInt(s[2]), Integer.parseInt(s[3]));
		 // buy and sell index fall within different months (filenames)
		 else
			 System.exit(-1);
